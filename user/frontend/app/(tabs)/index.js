...
export default function Home() {
  ...
  useCallback(() => {
    ...
    const loadDataOnFocus = async () => {
      try {
        ...
        // Load pickup location - only if changed from homepage
        const saved = await AsyncStorage.getItem("pickupDetails");
        if (saved) {
          const data = JSON.parse(saved);
          // ✅ Only update if from === "homepage" or no from flag
          if ((data.from === "homepage" || !data.from) && data.address) {
            setPickupAddress(data.address);
  ...
  const onUserSelectPickup = async (address) => {
    ...
    setUserSelectedPickup(true);
    await AsyncStorage.setItem("pickupDetails", JSON.stringify({ userSelected: true, address, from: "homepage" }));
  };
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            router.push({
              ...
              params: { address: pickupAddress, onSelect: onUserSelectPickup, from: "homepage" },
            })
          }
        >
...
const styles = StyleSheet.create({
  ...
  greetingText: { fontSize: 24, fontWeight: '800', color: '#FFF', marginBottom: 8 },
